...
<div class="relative min-h-1/2">
	<div class="flex flex-col-reverse justify-around text-end h-[300px]">
	...
	<div class="ml-auto flex w-[calc(100%-1.75rem)] items-center justify-between">
		{#each $weeklyActivity as { date, formattedDate, tasksCompleted }}
			<div class="group flex flex-col items-center">
				<div
					class="absolute bottom-[40px] max-h-[calc(100%-1rem)] min-h-[0.5rem] w-1 rounded-full bg-accent-light"
				...
				<span
					...
					style="bottom: calc({(tasksCompleted / 25) * 100}% + 2.5rem)">{tasksCompleted} tasks</span
				>
			</div>
		{/each}
	</div>
