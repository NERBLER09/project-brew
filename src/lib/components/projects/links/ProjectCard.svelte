<script lang="ts">
	...
	export let invited_people: string[] = [];
	export let tags: string[] = [];
</script>

<section href="/app/projects/{id}" class="bg-grey-100 p-4 rounded-lg w-fit">
	<header>
		<h2 class="text-md font-medium text-grey-700">{project_name}</h2>
	</header>
	<div>
		<p class="text-grey-700 text-sm font-medium h-[4ch] overflow-hidden text-ellipsis">
			{description}
		</p>

		<div class="flex items-center gap-md flex-wrap pt-sm">
			{#each tags as tag}
